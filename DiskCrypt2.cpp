...
DiskCrypt2::DiskCrypt2(QWidget *parent) : QWidget(parent)
{
    layout = new QVBoxLayout(this);
    chooseOperation = new QButtonGroup(this);
    setProtectionRadio = new QRadioButton("Зашифрование в режиме SDE", this);
    setProtectionRadio->setGeometry(10,20,200,30);
    removeProtectionRadio = new QRadioButton("Расшифрование в режиме SDE", this);
    removeProtectionRadio->setGeometry(200,20,200,30);
...
//-----------------------------------------------------------------------------------------
    ...
    connect(openFileCriticalDataButton, &QPushButton::clicked, this, &DiskCrypt2::chooseCriticalDataFile);
    connect(fileCriticalDataRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleCriticalDataInput);
    connect(manualCriticalDataRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleCriticalDataInput);


    connect(openFileKeyButton, &QPushButton::clicked, this, &DiskCrypt2::chooseKeyFile);
    connect(fileKeyRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleKeyInput);
    connect(manualKeyRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleKeyInput);

    connect(resetButton, &QPushButton::clicked, this, &DiskCrypt2::resetForm);
    connect(encryptButton, &QPushButton::clicked, this, &DiskCrypt2::encryptData);

    connect(openFileTitleButton, &QPushButton::clicked, this, &DiskCrypt2::chooseTitleFile);
    connect(fileTitleRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleTitleInput);
    connect(manualTitleRadio, &QRadioButton::toggled, this, &DiskCrypt2::toggleTitleInput);
}

void DiskCrypt2::chooseCriticalDataFile()
{
...
void DiskCrypt2::chooseTitleFile()
{
...
void DiskCrypt2::toggleCriticalDataInput()
{
...
void DiskCrypt2::toggleKeyInput()
{
...
void DiskCrypt2::toggleTitleInput()
{
...
void DiskCrypt2::encryptData()
{
    ...
    {
        criticalDataSizeBytes = byteCriticalDataArray.size();
        if(criticalDataSizeBytes < 32)
            {
                QMessageBox::warning(this, "Ошибка", "Данные должны занимать больше 32 байт");
                return;
            }
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    if(manualTitleRadio->isChecked())
    {
        ...
        {
                QMessageBox::warning(this, "Ошибка", "Синхропосылка должена быть ровно 16 байт.");
                return;
        }
    }
    ...
    if(setProtectionRadio->isChecked())
    {
        beltSDEEncr(result,criticalblock,criticalDataSizeBytes,key,32,title);
        QString output = printByteArray(result,criticalDataSizeBytes);
        OutPutLabel->setText(output);
    }
    if(removeProtectionRadio->isChecked())
    {
        beltSDEDecr(result,criticalblock,criticalDataSizeBytes,key,32,title);
        QString output = printByteArray(result,criticalDataSizeBytes);
        OutPutLabel->setText(output);
    }

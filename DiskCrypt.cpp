...
DiskCrypt::DiskCrypt(QWidget *parent) : QWidget(parent)
{
    layout = new QVBoxLayout(this);
    chooseOperation = new QButtonGroup(this);
    setProtectionRadio = new QRadioButton("Зашифрование в режиме BDE", this);
    setProtectionRadio->setGeometry(10,20,200,30);
    removeProtectionRadio = new QRadioButton("Расшифрование в режиме BDE", this);
...
//-----------------------------------------------------------------------------------------
    ...
    connect(openFileCriticalDataButton, &QPushButton::clicked, this, &DiskCrypt::chooseCriticalDataFile);
    connect(fileCriticalDataRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleCriticalDataInput);
    connect(manualCriticalDataRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleCriticalDataInput);


    connect(openFileKeyButton, &QPushButton::clicked, this, &DiskCrypt::chooseKeyFile);
    connect(fileKeyRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleKeyInput);
    connect(manualKeyRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleKeyInput);

    connect(resetButton, &QPushButton::clicked, this, &DiskCrypt::resetForm);
    connect(encryptButton, &QPushButton::clicked, this, &DiskCrypt::encryptData);

    connect(openFileTitleButton, &QPushButton::clicked, this, &DiskCrypt::chooseTitleFile);
    connect(fileTitleRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleTitleInput);
    connect(manualTitleRadio, &QRadioButton::toggled, this, &DiskCrypt::toggleTitleInput);
}

void DiskCrypt::chooseCriticalDataFile()
{
...
void DiskCrypt::chooseTitleFile()
{
...
void DiskCrypt::toggleCriticalDataInput()
{
...
void DiskCrypt::toggleTitleInput()
{
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if(setProtectionRadio->isChecked())
    {
        beltBDEEncr(result,criticalblock,criticalDataSizeBytes,key,32,title);
        QString output = printByteArray(result,criticalDataSizeBytes);
        OutPutLabel->setText(output);
    }
    if(removeProtectionRadio->isChecked())
    {
        beltBDEDecr(result,criticalblock,criticalDataSizeBytes,key,32,title);
        QString output = printByteArray(result,criticalDataSizeBytes);
        OutPutLabel->setText(output);
    }
